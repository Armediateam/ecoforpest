...
class ViewProposal extends ViewRecord
{
    ...
    protected static string $view = 'filament.resources.proposal-resource.pages.view-proposal-tabs';
    ...
    {
        $rawConfig = ResourceTabsConfiguration::forProposal()->toArray();
        $processedTabs = [];
    ...
    // public function getTabsConfiguration(): array
    // {
    //     return ResourceTabsConfiguration::forProposal()->toArray();
    // }
    ...
    protected function getHeaderActions(): array
    {
        return [
            ...
            Actions\Action::make('downloadPdf')
                ...
                ->url(fn($record) => route('proposals.preview', ['proposal' => $record->id]))
                ->openUrlInNewTab(),
            Actions\Action::make('add_comment')
                ...
                ->action(function (array $data): void {
                    ...
                    activity()
                        ...
                        ->log("{$comment->user->name} menambahkan komentar pada Proposal {$this->record->subject}");
            ...
            Actions\Action::make('approve')
                ...
                ->action(function ($record) {
                    $proposal = Proposal::find($record->id);
                    ...
                    foreach ($adminUsers as $admin) {
                        Notification::make()
                            ...
                            ->body('The proposal ' . $record->lead?->name . ' has been approved.')
                            ->success()
                            ->send();
                        ...
                        Notification::make()
                            ...
                            ->actions([
                                Action::make('view')
                                    ->label('View Proposal')
                                    ->url(route('filament.secret.resources.proposals.view', ['record' => $record->id])),
                            ])
                            ->sendToDatabase($admin);
                    }

                    return redirect()->to('secret/proposals');
                }),
                ...
                ->action(function ($record) {
                    ...
                    foreach ($adminUsers as $admin) {
                        Notification::make()
                            ...
                            ->body('Proposal for ' . $record->lead?->name . ' has been rejected.')
                            ->danger()
                            ->send();
